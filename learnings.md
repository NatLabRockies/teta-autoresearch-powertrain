...
- **Fast GPU MLP ensemble member is now practical.** A small (256→128) PyTorch MLP trained ~60 epochs/Adam-lr=1e-3/batch=8192 fits in ~25–55s on a P100 and adds -0.061% RMSE to the 10-seed LGBM ensemble at 1/(N+1) weight (bev-apr27/exp1+exp2). Fits within the 600s per-run cap; reopens the MLP-member direction that bev-apr24c had closed (slow sklearn version was -0.14% but at 1054s, over-budget).
- **MLP-as-ensemble-member tuning has a flat plateau at the simple Adam/MSE baseline.** Confirmed dead-ends at the LGBM+MLP blend optimum: cosine-annealing LR, dropout 0.1, AdamW(weight_decay=1e-4), Huber loss(delta=1.0), MLP blend weight 1→2 (1/11→2/12). All tied within run-to-run variance (~±0.02% / ±20s wall). BatchNorm1d actively hurts (+0.95%) — eval-mode running stats diverge from training-mode batch stats on this heavy-tailed target. Run-to-run wall-time variance is ~±20s on the GPU+CPU mixed pipeline; deltas ≤0.02% should be treated as noise.
...
- **MLP-as-ensemble-member tuning is plateaued at the bare Adam/MSE config** (resolved bev-apr27/exp3–exp11). Once the (256,128) GPU MLP at Adam-lr=1e-3/MSE/60 epochs/batch=8192 is in place, none of the natural follow-ups move the needle: cosine LR (tied), dropout 0.1 (tied), AdamW wd=1e-4 (tied), Huber delta=1.0 (tied), blend weight 1→2 (tied), and BatchNorm1d actively regresses +0.95% (eval-mode running stats vs heavy-tailed target). MLP architecture/regularization tuning is exhausted — open angles are multi-MLP seed-averaging (compute risk) and stacking, not single-MLP knobs.

### Best known config

- **RMSE:** 0.006521 (bev-apr27/exp2, 10-seed LGBM ensemble + 1 GPU MLP member at 1/11 weight, 24 features).
- Prior bests: 0.006527 (bev-apr24c/exp39, LGBM-only) → 0.006562 (bev-apr24b/exp30) → 0.006644 (bev-apr24/exp38, single-model).
- Session `bev-apr27` (in progress): **-0.092%** (0.006527 → 0.006521) so far. Wins: GPU MLP ensemble member with 25 epochs (-0.031%, exp1) → 60 epochs (-0.061% cumulative, exp2). Subsequent 9 experiments all tied or regressed.
...
- Multi-MLP seed averaging (e.g. 2 MLPs at 30 epochs each, blended 1/12 each) — would test whether decorrelation helps the way it does for LGBM. Wall-time-tight: 2x ~25s for 30-epoch MLPs ≈ same as one 60-epoch MLP, so probably feasible.
...
- Trade-off: lower `n_estimators` (e.g. 1300) to free ~10–15% LGBM wall time, then test feature additions or a bigger MLP. Not yet attempted — `n_estimators=1500` is the proven optimum from `bev-apr24c`, so trimming might slightly hurt LGBM but free budget for compounding gains elsewhere.
